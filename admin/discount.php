...
<tfoot>
    ...
    $select=mysqli_query($conn,"select * from `discount` d inner join `products` p on p.product_id=d.product_id order by d.product_id desc");
    while($dis=mysqli_fetch_assoc($select)){
        ?>
        <tr>
            <td><?php echo $dis['product_name']; ?></td>
            <td><?php echo $dis['discount_price']; ?></td>
            <td><?php echo $dis['price']; ?></td>
            <td><?php echo $dis['end_date']; ?></td>
        </tr>
    <?php
    }
    ?>
